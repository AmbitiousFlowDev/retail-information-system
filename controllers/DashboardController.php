...
final class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $userFullName = $this->getLoggedInUserFullName($userId);
    ...
    private function getCurrentUserId(): int
    {
        $user = $_SESSION['user'];
        if (is_object($user)) {
            return method_exists($user, 'getId') ? $user->getId() : (int) ($user->user_id ?? 0);
        }
    ...
    {
        $db = Connection::getInstance();
        return (int) $db->query("SELECT COUNT(*) FROM `Order`")->fetchColumn();
    }

    private function getProductsCount(): int
    {
        $db = Connection::getInstance();
        return (int) $db->query("SELECT COUNT(*) FROM Product")->fetchColumn();
    }

    private function getClientsCount(): int
    {
        $db = Connection::getInstance();
        return (int) $db->query("SELECT COUNT(*) FROM Client")->fetchColumn();
    }
